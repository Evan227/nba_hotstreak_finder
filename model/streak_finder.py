...
def create_player_average_df():
    ...
    std = top_players.std()
    standard_deviation_pts = 1.5 * std.PTS
    standard_deviation_fg3m = 1 * std.FG3M

    return top_players, list_of_players, standard_deviation_pts, standard_deviation_fg3m


def find_hot_or_cold_streak_players():
    print("creating season average df...")
    average_full_season, list_of_players, std_pts, std_fg3m = create_player_average_df()
    ...
    for player in list_of_players:
        ...
        if (players_recent_average_pts > players_average_pts + std_pts and players_recent_fg_percen > 0.5) \
                or (players_recent_average_pts < players_average_pts - std_pts and players_recent_fg_percen < 0.45):
            """
            ...
            d = {
                ...
                "REB": "{}".format(round(player_season_average.REB.item(), 2)),
                "AST": "{}".format(round(player_season_average.AST.item(), 2)),
                "TEAM": player_team,
                "NEXT_GAME_OPP": next_opp_team,
                "HOT_OR_COLD": ("HOTSTREAK:" if players_recent_average_pts > players_average_pts else "COLDSTREAK:")
            }
            HOT_STREAK_PLAYERS_PTS.append(d)
        elif (players_recent_average_3pm > players_average_3pm + std_fg3m and players_recent_3p_percen > 0.35)\
                or (players_recent_average_3pm < players_average_3pm - std_fg3m and players_recent_3p_percen < 0.3):
            """
            ...
            d = {
                ...
                "TEAM": player_team,
                "NEXT_GAME_OPP": next_opp_team,
                "HOT_OR_COLD": ("HOTSTREAK:" if players_recent_average_3pm > players_average_3pm else "COLDSTREAK:")
            }
...
def print_hotstreak_players(player_arr):
    for p in player_arr:
        ...
        if p.get("REB") and p.get("AST"):
            print("They are also averaging {} rebounds and {} assists in the last 3 games".format(
                p["REB"],
                p["AST"]
            ))

        try:
            if p["NEXT_GAME_OPP"] == "N/A":
                raise Exception
